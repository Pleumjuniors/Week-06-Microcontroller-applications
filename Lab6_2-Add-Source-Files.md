...
แยกไฟล์ทำให้โค้ดอ่านง่าย แก้ง่าย ไม่ต้องรวมทุกอย่างไว้ไฟล์เดียว
2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
เวลาเพิ่ม source ใหม่ ต้องไปแก้ CMakeLists.txt ให้เพิ่มไฟล์นั้นเข้า build list
3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
ไว้ประกาศฟังก์ชัน/ตัวแปร ให้ไฟล์อื่นเรียกใช้ได้
ถ้าไม่มี .h ไฟล์อื่นจะไม่รู้ว่าฟังก์ชันนั้นมีอยู่
4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
ต้องระบุ INCLUDE_DIRS เพื่อบอก CMake ว่า header อยู่ตรงไหน จะได้ include เจอ
5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
กันไม่ให้ไฟล์ build, binary, config ส่วนตัว หลุดเข้า repo → repo สะอาด ไม่บวม
6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
ใช้ task แยก LED ออกมา ทำให้มันกระพริบ/ทำงานอิสระ ไม่ไปรบกวน logic อื่น
7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
แยก sensor, display, led เป็นโมดูล ช่วยให้โค้ดเป็นระเบียบ ดูแล/แก้ง่าย และ reuse ได้
